...
trackerUrl = "http://api.yeahthattrolley.com/api/v1"
# Create your views here.
@xframe_options_exempt
def track(request):
	#get the list of currently active routes
	activeroutes = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Routes/Active")).read().decode('utf-8'))
	routes = []

	#get the list of schedules in case there's no trolleys running
	schedules = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/RouteSchedules")).read().decode('utf-8'))
	...
	routenames = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Routes")).read().decode('utf-8'))
	...
	for route in activeroutes:
		...
		routes.append(json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Routes/" + str(route['ID']))).read().decode('utf-8')))
...
def update(request):
	trolleys = []
	activetrolleys = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Trolleys/Running")).read().decode('utf-8'))
	return HttpResponse(json.dumps(activetrolleys))

@xframe_options_exempt
def schedule(request, scheduleid):
	schedules = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/RouteSchedules")).read().decode('utf-8'))
	scheduleroute = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Routes/" + scheduleid)).read().decode('utf-8'))
	routenames = json.loads(urllib.request.urlopen(urllib.request.Request(trackerUrl + "/Routes")).read().decode('utf-8'))
